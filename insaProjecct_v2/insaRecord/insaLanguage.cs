...
{
    public partial class insaLanguage : Form, Iinsa_Interface
    {
        ...
        public void ShowData()
        {
            ...
            if (_DB.GetConnection() == true)
            {
                using (OracleCommand cmd = new OracleCommand())
                {
                    ...
                    cmd.CommandText = "select FORL_CODE, FORL_SCORE, FORL_ACQDATE, FORL_ORGAN from thrm_forl_hwy where FORL_EMPNO='" + insaSide.select_empno + "'";
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            dataGridView1.Rows.Add(reader["FORL_CODE"], reader["FORL_SCORE"], reader["FORL_ACQDATE"], reader["FORL_ORGAN"], "");
                        }
                    }
                }
            }
        ...
        public void gird_data_binding()
        {
            ...
            {
                String FORL_CODE = dtRow.Cells["외국어코드"].FormattedValue.ToString();
                String FORL_SCORE = dtRow.Cells["점수"].FormattedValue.ToString();
                String FORL_ACQDATE = dtRow.Cells["취득일"].FormattedValue.ToString();
                String FORL_ORGAN = dtRow.Cells["발급기관"].FormattedValue.ToString();
                String check = dtRow.Cells["정보상태"].FormattedValue.ToString();

                if (check.Equals("Insert"))
                {
                    thrm_add(insaSide.select_empno, FORL_CODE, FORL_SCORE, common.ParseString(FORL_ACQDATE, "yyyyMMdd"), FORL_ORGAN);
                }
                else if (check.Equals("Update"))
                {
                    thrm_update(insaSide.select_empno, FORL_CODE, FORL_SCORE, common.ParseString(FORL_ACQDATE, "yyyyMMdd"), FORL_ORGAN);
                }
            }
        ...
        public int thrm_add(String empno, String car_com, String car_region, DateTime car_yyyymm_f, String car_yyyymm_t)
        {
            ...
            try
            {
                if (_DB.GetConnection() == true)
                {
                    ...
                    comm.CommandText = @"INSERT INTO thrm_forl_hwy values(:empno, :resno, :name, :cname, :ename)";
        ...
        public int thrm_update(String empno, String car_com, String car_region, DateTime car_yyyymm_f, String car_yyyymm_t)
        {
            ...
            try
            {
                if (_DB.GetConnection() == true)
                {
                    ...
                    {
                        comm.Connection = _DB.Connection;
                        comm.CommandText = @"update thrm_forl_hwy set FORL_CODE='" + car_com + "', FORL_SCORE='" + car_region + "', FORL_ACQDATE='" + car_yyyymm_f.ToString("yyyyMMdd") + "', FORL_ORGAN='" + car_yyyymm_t + "' where FORL_EMPNO='" + empno + "' and FORL_CODE='" + car_com + "'";
                        var a = comm.ExecuteNonQuery();
                        check = 0;
        ...
        public int thrm_delete(String empno, String CAR_COM)
        {
            ...
            try
            {
                if (_DB.GetConnection() == true)
                {
                    ...
                    {
                        comm.Connection = _DB.Connection;
                        comm.CommandText = @"delete from thrm_forl_hwy where FORL_EMPNO='" + empno + "' and FORL_CODE='" + CAR_COM + "'";
                        var a = comm.ExecuteNonQuery();
                        check = 0;
                        Console.WriteLine(comm.CommandText);
        ...
        private void insaLanguage_Load(object sender, EventArgs e)
        {
